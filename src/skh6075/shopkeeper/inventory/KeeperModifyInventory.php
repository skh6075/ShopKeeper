...
class KeeperModifyInventory extends BaseInventory{
    ...
    public function onOpen(Player $who): void{
         ...
         * @priority HIGHEST
         */
        foreach ($this->keeper->getItmes() as $index => $item) {
            $buyItem = Item::jsonDeserialize($item [0]);
            $sellItem = Item::jsonDeserialize($item [1]);
            $buySlot = $index + 9;
            $sellSlot = $buySlot + 9;
            if ($buySlot >= 16) {
                break;
            }
            $this->setItem($buySlot, $buyItem);
            $this->setItem($sellSlot, $sellItem);
        }
    ...
    public function onClose(Player $who): void{
        ...
        for ($i = 0; $i < 8; $i ++) {
            $buyItem = $this->getItem(($buySlot = $i + 9));
            $sellItem = $this->getItem($buySlot + 9);
            if ($buyItem->isNull() or $sellItem->isNull()) {
                continue;
            }
            $this->keeper->setItems($i, [
                $buyItem->jsonSerialize(),
                $sellItem->jsonSerialize()
            ]);
        }
